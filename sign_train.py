...
def get_train_test_loader(data_dir,batch_size,augment,random_seed,test_size=0.1,shuffle=True,num_workers=4,pin_memory=False):
	...
	if augment:
		train_transform =torchvision.transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor(),transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
	else:
	...
	print('Num test images: ', len(valid_sampler))
...
try:
	optimizer = optim.SGD(model.classifier.parameters(), lr=0.01, momentum=0.85,weight_decay=0.001)
	print("VGG Loaded")
except:
	print("ResNet Loaded")
...
for epoch in range(n_epochs):
    ...
    for batch_i, (data, target) in enumerate(train_loader):
        ...
        for i in range(len(target)):
            ...
            class_total[label] += 1
        if batch_i % 32 == 31:
            print('Epoch %d, Batch %d loss: %.16f -- Acurracy: %.16f%%' %(epoch+1, batch_i + 1, train_loss / 32,100.0 * np.sum(class_correct) / np.sum(class_total)))
            train_loss = 0.0
            class_correct = list(0. for i in range(len(classes)))
            class_total = list(0. for i in range(len(classes)))
